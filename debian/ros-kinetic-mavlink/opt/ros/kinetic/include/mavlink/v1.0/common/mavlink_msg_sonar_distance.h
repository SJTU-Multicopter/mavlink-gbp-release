...
#define MAVLINK_MSG_ID_SONAR_DISTANCE_MIN_LEN 28
#define MAVLINK_MSG_ID_212_LEN 28
#define MAVLINK_MSG_ID_212_MIN_LEN 28
						       ...
						       float sonar_front, float sonar_behind, float sonar_left, float sonar_right, float sonar_up, float sonar_down, float sonar_cam)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_SONAR_DISTANCE_MIN_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_CRC);
}
						           ...
						           float sonar_front,float sonar_behind,float sonar_left,float sonar_right,float sonar_up,float sonar_down,float sonar_cam)
{
	...
	msg->msgid = MAVLINK_MSG_ID_SONAR_DISTANCE;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_SONAR_DISTANCE_MIN_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_CRC);
}
...
static inline void mavlink_msg_sonar_distance_send(mavlink_channel_t chan, float sonar_front, float sonar_behind, float sonar_left, float sonar_right, float sonar_up, float sonar_down, float sonar_cam)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SONAR_DISTANCE, buf, MAVLINK_MSG_ID_SONAR_DISTANCE_MIN_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_CRC);
#else
	mavlink_sonar_distance_t packet;
	packet.sonar_front = sonar_front;
	packet.sonar_behind = sonar_behind;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SONAR_DISTANCE, (const char *)&packet, MAVLINK_MSG_ID_SONAR_DISTANCE_MIN_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_sonar_distance_send_struct(mavlink_channel_t chan, const mavlink_sonar_distance_t* sonar_distance)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_sonar_distance_send(chan, sonar_distance->sonar_front, sonar_distance->sonar_behind, sonar_distance->sonar_left, sonar_distance->sonar_right, sonar_distance->sonar_up, sonar_distance->sonar_down, sonar_distance->sonar_cam);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SONAR_DISTANCE, (const char *)sonar_distance, MAVLINK_MSG_ID_SONAR_DISTANCE_MIN_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_LEN, MAVLINK_MSG_ID_SONAR_DISTANCE_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_SONAR_DISTANCE_LEN? msg->len : MAVLINK_MSG_ID_SONAR_DISTANCE_LEN;
        memset(sonar_distance, 0, MAVLINK_MSG_ID_SONAR_DISTANCE_LEN);
	memcpy(sonar_distance, _MAV_PAYLOAD(msg), len);
#endif
}
